import binascii, struct, logging
...
def Bcd2Str(bcd):
    # log.info("-----\n" + binascii.hexlify(bcd).decode('latin'))
...
def Bcd2Int(bcd):
    try:
        result = int(Bcd2Str(bcd))
    except:
        result = int(Bcd2Str(bcd),16)
        result = str(result)[:6]
    return result
...
class Iso8583:
    ...
    ValidContentTypes = ('a', 'n', 's', 'an', 'as', 'ns', 'ans', 'ansb', 'b', 'z')
    ...
    def ParseField(self, field, p):       
        ...
        try:
            ...
            raise ParseError("Cannot parse P-{0} - Invalid length: {1}".format(field, ex))

        if(Len > MaxLength):
            raise ParseError("P-{0} is larger than maximum length ({1}>{2})".format(field, Len, MaxLength))
        ...
        try:
            ...
            raise ParseError("Cannot parse P-{}: {}".format(field, str(ex))) from None
    ...
    def BuildField(self, field):
        try:
            ...
            raise SpecError("Cannot parse P-{0}: Incomplete field specification".format(field))
        ...
        if(LenType == LT.FIXED):
            ...
            if(Len > MaxLength):
                raise BuildError("Cannot Build P-{0}: Field Length larger than specification".format(field))            
            if(LenType == LT.LVAR):
        ...
        for field in sorted(self.__Bitmap):
            if(field != 1 and self.Field(field) == 1):
                try:
                    ...
                    raise type(ex)('Error building P-{}: '.format(field) + repr(ex)) from None                
        return self.__iso
    ...
    def FieldData(self, field, Value = None):
        if(Value == None):
            ...
            if(len(str(Value)) > self.__IsoSpec.MaxLength(field)):
                raise ValueError('Value length larger than field maximum ({0} - {1} - {2})'.format(self.__IsoSpec.MaxLength(field), len(str(Value)), str(Value)))
